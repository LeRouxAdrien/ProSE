 ...
 *  included p_printf() to display in color
 *  included extra-delay option card access slow down
 * 
 * version 1.60 / paulvh / Augustus 2017
 *  Fixed number of bugs and code clean-up.
 *  extended command line options
 *  included support for value block handling
 * 
 * version 1.65.3 / paulvh / October 2017
 *  added command line write_message option ( -N -B -M -H -A -T -G)
...
int block_action()
{
	...
	if ((addr =get_block_number(max_blocks-1,1)) == 0)  Scanner_free(0);
...
int change_block_access() 
{
	...
	p_printf (YELLOW,"Max %d. (0 = exit) : ", max_blocks-1);
	if ((addr = get_block_number(max_blocks-1,0)) == 0 ) Scanner_free(0);
	    ...
	    if (read_tag_str(addr, buff) == TAG_OK)
	    {
			...
			if (i != 32 )
			{
				...
				do
			    {
			        ...
			        else if (strcasecmp(ans,"exit") == 0)  Scanner_free(0);
...
void Scanner_new()	{
    ...
    if (HW_init(spi_speed,gpio)) Scanner_free(1);
    
    /* initialise the RC522 */
	InitRc522();

    /* read & set GID and UID from config file */
    if (read_conf_uid()!= 0) Scanner_free(1);
    
}

/* reset and terminate program */
void Scanner_free(int ex_code)
{
...
int main(int argc, char *argv[]) {
    ...
    while (loop > 0)
    {
        ...
        if (d_read){ if (read_from_card() != TAG_OK) Scanner_free(1); }
        
        /* write a specific card block */
        else if (d_write){ if(write_to_card() != TAG_OK) Scanner_free(1); }
        
        /* perform action based on info in a specific card block */
        else if (add_act){ if (block_action()!= TAG_OK) Scanner_free(1); }
        
        /* change access right on a specific card block */
        else if (change){ if (change_block_access()!= TAG_OK) Scanner_free(1); }
        
        /* update a key on specific sector */
        else if (upd_key){ if (key_upd()!= TAG_OK) Scanner_free(1);}
        
        /* read a specific card block */
        else if (create_block) {if (create_value_block(set_val)!= V_OK) Scanner_free(1);} 
	
        /* increment specific card block */
        else if (incr_block){ if(increment_value_block()!= V_OK) Scanner_free(1); }
        
        /* decrement specific card block */
        else if (decr_block){ if (decrement_value_block()!= V_OK) Scanner_free(1); }
        
        /* remove specific card block */
        else if (rem_block){ if (remove_value_block() != V_OK) Scanner_free(1); }
 
         /* show specific card block */
        else if (show_block){ if (show_value_block(&block, &value,1) != V_OK) Scanner_free(1); }
        
        /* perform action based on UID */
        else if (action) { if (uid_action()!= TAG_OK) Scanner_free(1); }

		// save memory of card ??
        else if (save_mem) {if (read_card_to_file()) Scanner_free(1); }
        ...
        // close / disconnect the card
        PcdHalt();  // Je ne sais pas à quoi sert cette fonction :)      
