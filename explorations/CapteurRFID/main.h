...
/* setup signals */
void set_signals();		//UTILE

/* catch signals to close out correctly */
void signal_handler(int sig_num);

/* perform init /reset BCM and GPIO */
uint8_t HW_init(uint32_t spi_speed, uint8_t gpio); //UTILE

/* reset and terminate the pogram */
void Scanner_free(int ex_code);	//UTILE

/* print message in color */
void p_printf (int level, char *format, ...);	//UTILE

/* read value from config file*/
int get_config_file();	//UTILE
...
int get_number();	//UTILE

/* get string */
void get_str(char * buf, int len);	//UTILE

/* get single (uppercase) character */
char get_charc();	//UTILE

/* check that string has valid number sequence */
int check_num(char *arg);	//UTILE
...
int ascii_to_hex(char *line, char *data, int filter_eof);	//UTILE
